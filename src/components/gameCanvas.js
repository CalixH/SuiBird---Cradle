import React, { useEffect, useState, useRef } from "react";
import skyImage from "../assets/images/sky.png";
import birdImage from "../assets/images/birdWings.png";
import landImage from "../assets/images/land.png";
import logoImage from "../assets/images/logo.png";
import pipeUpImage from "../assets/images/pipe-up.png";
import pipeDownImage from "../assets/images/pipe-down.png";
import pipeImage from "../assets/images/pipe.png";
import replayImage from "../assets/images/replay.png";
import defaultImage from "../assets/images/bird.png";
import {
  useRecoilState,
  useRecoilValue,
  useSetRecoilState,
  useRecoilCallback,
} from "recoil";
import {
  playerDeathState,
  playerScoreState,
  playingState,
  selectedNFTState,
} from "../store/atoms";

const GameCanvas = () => {
  const canvasRef = useRef(null);
  const [canvas, setCanvas] = useState();
  const [gameStarted, setGameStarted] = useState(false);
  const setScore = useSetRecoilState(playerScoreState);
  const setDeath = useSetRecoilState(playerDeathState);
  const updatePlayingState = useSetRecoilState(playingState);
  var playing = useRef(false);
  var charImageUrl = useRef(null);

  const getPlayingState = useRecoilCallback(
    ({ snapshot }) =>
      async () => {
        playing.current = await snapshot.getPromise(playingState);
      },
    []
  );

  const getCharImage = useRecoilCallback(
    ({ snapshot }) =>
      async () => {
        charImageUrl.current = await snapshot.getPromise(selectedNFTState);
      },
    []
  );

  const [ctx, setCtx] = useState();
  var width, height, birdPos;
  var sky,
    land,
    bird,
    pipe,
    pipeUp,
    pipeDown,
    scoreBoard,
    ready,
    splash,
    charImg;
  var dist, birdY, birdF, birdN, birdV;
  var animation, death, deathAnim;
  var pipes = [],
    pipesDir = [],
    pipeSt,
    pipeNumber;
  var score, maxScore;
  var dropSpeed;
  var mode, delta;
  var playend = false,
    playdata = [];

  const jumpIfPlaying = async (e) => {
    await getPlayingState();
    console.log("ready to go?", playing.current);
    if (playing.current) {
      jump();
      e.preventDefault();
    }
  };

  const initCanvas = () => {
    canvas.width = width = window.innerWidth;
    canvas.height = height = window.innerHeight;
    if (is_touch_device()) {
      canvas.addEventListener(
        "touchend",
        function (e) {
          e.preventDefault();
        },
        false
      );
      canvas.addEventListener("touchstart", jumpIfPlaying, false);
    } else canvas.onmousedown = jumpIfPlaying;
    window.onkeydown = jumpIfPlaying;
    loadImages();
  };

  const is_touch_device = () => {
    try {
      document.createEvent("TouchEvent");
      return true;
    } catch (e) {
      return false;
    }
  };

  const loadImages = () => {
    var imgNumber = 8,
      imgComplete = 0;
    var onImgLoad = function () {
      imgComplete++;
      if (imgComplete == imgNumber) {
        death = 1;
        dist = 0;
        birdY = (height - 112) / 2;
        birdF = 0;
        birdN = 0;
        birdV = 0;
        birdPos = width * 0.35;
        score = 0;
        pipeSt = 0;
        pipeNumber = 10;
        pipes = [];
        pipesDir = [];
        for (var i = 0; i < 10; ++i) {
          pipes.push(Math.floor(Math.random() * (height - 300 - delta) + 10));
          pipesDir.push(Math.random() > 0.5);
        }
        drawCanvas();
      }
    };

    sky = new Image();
    sky.src = skyImage;
    sky.onload = onImgLoad;

    land = new Image();
    land.src = landImage;
    land.onload = onImgLoad;

    bird = new Image();
    bird.src = birdImage;
    bird.onload = onImgLoad;

    pipe = new Image();
    pipe.src = pipeImage;
    pipe.onload = onImgLoad;

    pipeUp = new Image();
    pipeUp.src = pipeUpImage;
    pipeUp.onload = onImgLoad;

    pipeDown = new Image();
    pipeDown.src = pipeDownImage;
    pipeDown.onload = onImgLoad;

    ready = new Image();
    ready.src = replayImage;
    ready.onload = onImgLoad;

    charImg = new Image();
    charImg.src = defaultImage;
    charImg.onload = onImgLoad;
  };

  const drawCanvas = () => {
    clearCanvas();
    drawSky();
    for (var i = pipeSt; i < pipeNumber; ++i) {
      drawPipe(width - dist + i * 220, pipes[i]);
      if (mode == 2) {
        if (pipesDir[i]) {
          if (pipes[i] + 1 > height - 300) {
            pipesDir[i] = !pipesDir[i];
            pipes[i] -= 1;
          } else pipes[i] += 1;
        } else {
          if (pipes[i] - 1 < 10) {
            pipesDir[i] = !pipesDir[i];
            pipes[i] += 1;
          } else pipes[i] -= 1;
        }
      }
    }
    drawLand();
    drawBird();
    setScore(score);
  };

  const drawScore = () => {
    ctx.font = '40px "Righteous"';
    ctx.lineWidth = 5;
    ctx.strokeStyle = "#fff";
    ctx.fillStyle = "#000";
    var txt = "" + score;
    ctx.strokeText(txt, width / 12, height * 0.25);
    ctx.fillText(txt, width / 12, height * 0.25);
  };

  const deathAnimation = () => {
    if (splash) {
      ctx.drawImage(splash, width / 2 - 94, height / 2 - 54);
      splash = undefined;
    } else {
      ctx.drawImage(scoreBoard, width / 2 - 118, height / 2 - 54);
      playend = true;
      playdata = [mode, score];
    }
    ctx.drawImage(ready, width / 2 - 57, height / 2 + 10);
    maxScore = Math.max(maxScore, score);
  };

  const drawBird = async () => {
    //	ctx.translate(width * 0.35 + 17, birdY + 12);
    //	var deg = -Math.atan(birdV / 2) / 3.14159;
    //	ctx.rotate(deg);
    // ctx.fillRect(birdPos, birdY, bird.width, bird.height / 4);
    //

    await getCharImage();
    if (charImageUrl.current) {
      charImg.src = charImageUrl.current.url;
    }
    ctx.drawImage(
      charImg,
      0,
      0,
      charImg.width,
      charImg.height,
      birdPos,
      birdY,
      bird.width,
      bird.height / 4
    );
    ctx.drawImage(
      bird,
      0,
      birdN * 24,
      bird.width,
      bird.height / 4,
      birdPos,
      birdY,
      bird.width,
      bird.height / 4
    );

    //	ctx.rotate(-deg);
    //	ctx.translate(-width * 0.35 - 17, -birdY - 12);
    birdF = (birdF + 1) % 6;
    if (birdF % 6 == 0) birdN = (birdN + 1) % 4;
    birdY -= birdV;
    birdV -= dropSpeed;
    if (birdY + 138 > height) {
      clearInterval(animation);
      death = 1;
    }
    if (death) {
      updatePlayingState(false);
      deathAnimation();
    }
  };

  const jump = () => {
    if (death) {
      dist = 0;
      birdY = (height - 112) / 2;
      birdF = 0;
      birdN = 0;
      birdV = 0;
      death = 0;
      score = 0;
      birdPos = width * 0.35;
      pipeSt = 0;
      pipeNumber = 10;
      pipes = [];
      pipesDir = [];
      for (var i = 0; i < 10; ++i) {
        pipes.push(Math.floor(Math.random() * (height - 300 - delta) + 10));
        pipesDir.push(Math.random() > 0.5);
      }
      anim();
    }
    if (mode == 0) birdV = 6;
    else if (mode == 1) birdV = 6;
    else birdV = 6;
  };
  const drawLand = () => {
    var totWidth = -dist;
    while (totWidth < width) {
      ctx.drawImage(land, totWidth, height - 112);
      totWidth += land.width;
    }
    dist = dist + 2;
    var tmp = Math.floor(dist - width * 0.65) % 220;
    if (dist >= width * 0.65 && Math.abs(tmp) <= 1) {
      score++;
    }
  };
  const drawPipe = (x, y) => {
    ctx.drawImage(pipe, x, 0, pipe.width, y);
    ctx.drawImage(pipeDown, x, y);
    ctx.drawImage(pipe, x, y + 168 + delta, pipe.width, height - 112);
    ctx.drawImage(pipeUp, x, y + 144 + delta);
    if (
      x < birdPos + 32 &&
      x + 50 > birdPos &&
      (birdY < y + 22 || birdY + 22 > y + 144 + delta)
    ) {
      clearInterval(animation);
      death = 1;
    } else if (x + 40 < 0) {
      pipeSt++;
      pipeNumber++;
      pipes.push(Math.floor(Math.random() * (height - 300 - delta) + 10));
      pipesDir.push(Math.random() > 0.5);
    }
  };

  const drawSky = () => {
    var totWidth = 0;
    while (totWidth < width) {
      ctx.drawImage(sky, totWidth, height - 221);
      totWidth += sky.width;
    }
  };

  const anim = () => {
    animation = setInterval(drawCanvas, 1000 / 60);
  };

  const clearCanvas = () => {
    ctx.fillStyle = "#4EC0CA";
    ctx.fillRect(0, 0, width, height);
  };
  useEffect(() => {
    setCanvas(canvasRef.current);
  }, []);
  useEffect(() => {
    if (canvas) {
      setCtx(canvas.getContext("2d"));
    }
  }, [canvas]);
  useEffect(() => {
    if (ctx) {
      mode = 0;
      score = 0;
      playdata = [0, 0];
      maxScore = 0;
      dropSpeed = 0.3;
      mode = 0;
      delta = 100;
      initCanvas();
      setInterval(() => {
        if (death == 1) setDeath(true);
      }, 100);
    }
  }, [ctx]);
  window.onresize = function () {
    canvas.width = width = window.innerWidth;
    canvas.height = height = window.innerHeight;
    drawCanvas();
  };

  return (
    <canvas id="canvas" ref={canvasRef} className="flex w-full h-full"></canvas>
  );
};

export default GameCanvas;
